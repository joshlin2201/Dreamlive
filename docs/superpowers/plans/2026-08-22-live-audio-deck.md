...
- [x] Write failing tests for previous/restart, repeat boundaries, add-next/end, select, locked reorder, and locked removal.
- [x] Implement pure helpers returning the updated queue and current index together.
- [x] Route existing next-track behavior through the shared boundary helper.
- [x] Run the focused playlist/show-flow tests.
...
- [x] Write failing tests for spectrum bin aggregation, normalization, and idle output.
- [x] Insert one configured `AnalyserNode` between master gain and destination.
- [x] Implement a reusable canvas visualizer with one allocation, capped DPR, resize cleanup, 30fps cap, paused decay, and reduced-motion behavior.
- [x] Render compact BGM and focus-player variants from the same analyser reference.
- [x] Run spectrum tests and verify playback remains available when Web Audio is unavailable.
...
- [x] Consolidate font, spacing, radius, elevation, focus, motion, and semantic button tokens.
- [x] Normalize every button to primary, transport, secondary, ghost, status, or destructive styling with 40/48px geometry.
- [x] Reduce nested borders, oversized padding, empty canvas space, and decorative diagonal bands.
- [x] Replace browser-blue search focus with the shared rose focus treatment.
...
- [x] Run `CI=true npm test -- --watchAll=false`.
- [x] Run `npm run build`, `npm run cap:sync`, and `npm run verify:ios-release`.
- [x] Serve the production build and test with real MP3/M4A plus at least 300 library records.
- [x] Exercise import, search, keyboard selection, queue edits, seek, repeat, transport, both visualizers, live setup edits, return-to-live, pause/resume, and transition.
